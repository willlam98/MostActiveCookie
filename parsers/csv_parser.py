...
import csv
import datetime
from exceptions.customed_exception import CSVFormatError, ParsingError
...
class CSVParser(Parser):
    ...
    def parse(self, file):
        if not self.can_parse_file(file):
            raise ParsingError(Parser.FILE_TYPE_NOT_SUPPORTED_MESSAGE.format(self.FILE_TYPE))
    ...
    def __validate_row(self, row, row_number):
        ...
        try:
            ...
            raise CSVFormatError('Invalid timestamp found {} in row #{}, expecting {}'
    ...
    def __validate_fieldnames(self, fieldnames):
        if not fieldnames == self.LOG_FILE_FIELD_NAMES:
            raise CSVFormatError("Invalid log file header, expecting '{}'"
